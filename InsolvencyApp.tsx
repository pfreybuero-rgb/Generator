...
import { FileText, ScrollText, ShieldCheck, ArrowLeft } from 'lucide-react';

const STORAGE_KEY = 'impro_insolvenz_state_v1';

interface InsolvencyAppProps {
  onBack: () => void;
}

const InsolvencyApp: React.FC<InsolvencyAppProps> = ({ onBack }) => {
    ...
    <div className="flex h-screen overflow-hidden bg-slate-100 font-sans">
      <div className="w-[360px] flex-shrink-0 bg-white shadow-2xl z-10 h-full border-r border-slate-200 flex flex-col">
         <div className="p-5 border-b border-slate-800/10 bg-brand-navy text-white flex items-center justify-between">
            ...
            <button onClick={onBack} className="p-2 hover:bg-white/10 rounded-full transition-colors text-slate-400" title="Zurück zum Hauptmenü">
               <ArrowLeft size={18} />
            </button>
         </div>
